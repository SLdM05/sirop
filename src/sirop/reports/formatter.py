...
def _build_tp_part6_income_summary(
    ...
    for asset, income_type in sorted(totals.keys()):
        ...
        lines.append(f"| {asset} | {income_type} | {_fmt_units(units)} | {_fmt_cad(fmv)} |")
...
def build_report(  # noqa: PLR0913 PLR0915
    ...
    sections.append(
        "- **T1 Line 12700:** Enter the taxable capital gains figure from the Part A summary box."
    )
...
def build_detail_report(  # noqa: PLR0913
    ...
    sections.append("")
    sections.append(
        f"*Detail backup generated by sirop {sirop_version} \xb7 Data source: {batch_name}.sirop*"
    )
